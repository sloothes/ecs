...
(function (global, factory) {
		...
		_createClass(Octree, [{
			...
			value: function importThreeMesh(threeMesh) {
				...
				for (var _i = 0, _l = geometry.faces.length; _i < _l; _i++) {
					...
					var _face = new Face(
						geometry.vertices[geometry.faces[_i].a], 
						geometry.vertices[geometry.faces[_i].b], 
						geometry.vertices[geometry.faces[_i].c], 
						geometry.faces[_i].normal, geometryId
					);
					this.addFace(_face);
				}
			}
	...
	var AnimationController = function () {
		function AnimationController(object) {
			...
			for (var i = 0, l = this.object.animations.length; i < l; i++) {

				var anim = this.object.animations[i];
				this.motion[anim.name] = this.mixer.clipAction(anim, object);
		...
		_createClass$2(AnimationController, [{
			...
			value: function turn(rad, immediate) {
				...
				var that = this;
				var prevRotY = this.object.rotation.y;
				var targetRotY = rad;
				...
				if (immediate) {
					...
					this.object.rotation.y = targetRotY;
					return;
				}
...

				{
					...
					(function interval() {
						...
						if (now >= end) {
							...
							that.object.rotation.y = _targetRotY;
						...
						that.object.rotation.y = prevRotY + deltaY * progress;
					})();
				}
			}
	...
	var CharacterController = function (_EventDispatcher) {
		...
		function CharacterController(object3d, radius) {
			...
			_this.events = function () {
				...
				} else if (!wasRunning && _this.isRunning && !_this.isJumping && _this.isGrounded 
						   || !wasGrounded && _this.isGrounded && _this.isRunning 
						   || wasOnSlope && !_this.isOnSlope && _this.isRunning && _this.isGrounded) {
	...
	function onkeyup(event) {
		...
		if (!this.isUp && !this.isDown && !this.isLeft && !this.isRight 
			&& (event.keyCode === KEY_W || event.keyCode === KEY_UP 
				|| event.keyCode === KEY_S || event.keyCode === KEY_DOWN 
				|| event.keyCode === KEY_A || event.keyCode === KEY_LEFT 
				|| event.keyCode === KEY_D || event.keyCode === KEY_RIGHT)) {
