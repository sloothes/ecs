//	src="/ecsy-three/web_modules/ecsy.js"
...
//	performance.now() "polyfill"
	const now = hasWindow && typeof window.performance !== "undefined" 
...
//	@class EntityManager
	...
	//	@param {Object} values Optional values to replace the default attributes
		...
		entityAddComponent(entity, Component, values) {
			...
			if (Component.__proto__ === SystemStateComponent) entity.numStateComponents++;
				...
				//	Reactive configuration added/removed/changed
					...
					if (queryConfig.listen) {
						validEvents.forEach(eventName => {
							if (!this.execute) console.warn("System '"+this.constructor.name+"' has defined listen events ("
							+ validEvents.join()+") for query '"+queryName+"' but it does not implement the 'execute' method.");
	...
	function createType(typeDefinition) {
		...
		if (undefinedProperties.length > 0) {
			throw new Error(
				"createType expects a type definition with the "
				+"following properties: "+undefinedProperties.join(", ")
			);
		}
...
//	global Peer.
	...
	function includeRemoteIdHTML(remoteId) {
		...
		infoDiv.style.cssText = "align-items:center;background-color:#333;color:#aaa;display:flex;font-family:Arial;font-size:1.1em;"
		infoDiv.style.cssText += "height:40px;justify-content:center;left:0;opacity:0.9;position:absolute;right:0;text-align:center;top:0;";
